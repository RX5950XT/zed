...
impl TitleBar {
    ...
    fn render_remote_project_connection(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let (nickname, tooltip_title, icon) = match options {
            ...
            RemoteConnectionOptions::Wsl(_) => (None, "遠端專案", IconName::Linux),
            RemoteConnectionOptions::Docker(_dev_container_connection) => {
                (None, "開發容器", IconName::Box)
            }
            #[cfg(any(test, feature = "test-support"))]
            RemoteConnectionOptions::Mock(_) => (None, "模擬遠端專案", IconName::Server),
        };

        let nickname = nickname.unwrap_or_else(|| host.clone());

        let (indicator_color, meta) = match self.project.read(cx).remote_connection_state(cx)? {
            remote::ConnectionState::Connecting => (Color::Info, format!("連線中: {host}")),
            remote::ConnectionState::Connected => (Color::Success, format!("已連線: {host}")),
            remote::ConnectionState::HeartbeatMissed => (
                Color::Warning,
                format!("與 {host} 的連線嘗試逾時，正在重試…"),
            ),
            remote::ConnectionState::Reconnecting => (
                Color::Warning,
                format!("與 {host} 的連線中斷，重新連線中…"),
            ),
            remote::ConnectionState::Disconnected => {
                (Color::Error, format!("已中斷與 {host} 的連線"))
            }
        };
        ...
        Some(
            Button::new("project_owner_trigger", host_user.github_login.clone())
                ...
                .tooltip(move |_, cx| {
                    ...
                    Tooltip::with_meta(tooltip_title, None, "點擊以跟隨", cx)
                })
                .on_click({
    ...
    fn render_worktree_and_branch(
        ...
        let display_label: SharedString = if let Some(ref name) = creation_in_progress {
            if is_switch {
                format!("載入 {} 中…", name).into()
            } else {
                format!("建立 {} 中…", name).into()
            }
        } else {
        ...
        let branch_picker = branch_name.and_then(|branch_name| {
            settings.show_branch_name.then(|| {
                ...
                let trigger = if is_detached_head {
                    Button::new("project_branch_trigger", "建立分支")
                        .selected_style(ButtonStyle::Tinted(TintColor::Accent))
                        .label_size(LabelSize::Small)
                        .start_icon(
                ...
                PopoverMenu::new("branch-menu")
                    ...
                    .trigger_with_tooltip(trigger, move |_window, cx| {
                        let meta = if is_detached_head {
                            format!("分離的 HEAD: {}", branch_tooltip_label)
                        } else {
                            format!("目前已簽出: {}", branch_tooltip_label)
                        };
                        Tooltip::with_meta(
                            "分支與封存",
        ...
        match status {
            ...
            | client::Status::ReconnectionError { .. } => Some(
                div()
                    ...
                    .tooltip(Tooltip::text("已中斷連線"))
                    .into_any_element(),
            ),
            client::Status::UpgradeRequired => {
                let auto_updater = auto_update::AutoUpdater::get(cx);
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    Some(AutoUpdateStatus::Updated { .. }) => "請重新啟動 Zed 以繼續協作",
        ...
        PopoverMenu::new("user-menu")
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, |menu, _, _cx| {
                    ...
                    .action("設定", zed_actions::OpenSettings.boxed_clone())
                    .action("按鍵對應", Box::new(zed_actions::OpenKeymap))
                    .action(
                        "主題…",
                    ...
                    .when(ai_enabled, |menu| {
                        let fs = fs.clone();
                        menu.separator()
                            .submenu("面板配置", move |menu, _window, _cx| {
                                let fs = fs.clone();
